...
def run_sync_test():
    ...
    print("============================================================")
    print("  🔄 ADMIN PANEL SYNCHRONIZATION TEST (V4)")
    print("============================================================")
    ...
    try:
        ...
        print("[*] Using valid user story 'WF-7849: Structuring PF and PJ Deposit Collateral Products'...")
        
        js_update = """
        const callback = arguments[arguments.length - 1];
        
        const validUsKey = 'WF-7849: Structuring PF and PJ Deposit Collateral Products';
        ...
        if (usInput) {
            ...
            if (window.syncStateToCloud) {
                window.syncStateToCloud(validUsKey, 'manual', {'check-manual-0': true}, 85)
                    .then(() => callback('done'))
        ...
        driver.execute_async_script("""
            ...
            if (window.deleteStateFromCloud) {
                window.deleteStateFromCloud('WF-7849: Structuring PF and PJ Deposit Collateral Products').then(() => callback('done'));
            } else callback('done');
        """)
